...
describe('FormListItem', () => {
	...
	test( 'clicking on edit', () => {
		...
		component.find( '.' + FormListItem.defaultProps.classNames.edit + ' button').simulate( 'click' );
	...
	test( 'clicking on preview', () => {
		...
		component.find( '.' + FormListItem.defaultProps.classNames.preview + ' button').simulate( 'click' );
		expect(onFormAction.mock.calls[0][0]).toBe(formId);
